...
(function () {
  ...
  function countReviewPersonas(task) {
    var text = String(task && task.review_personas || '');
    var matches = text.match(/REVIEW_PERSONA:[A-Z0-9_]+/g) || [];
    return Array.from(new Set(matches)).length;
  }

  function countChangedFiles(task) {
    var text = String((task && (task.changed_files || task.updated_refs || task.goal_steps || '')) || '');
    var matches = text.match(/[\w./-]+\.[A-Za-z0-9][A-Za-z0-9_-]*/g) || [];
    return Array.from(new Set(matches.filter(function (token) {
      return token.indexOf('/') >= 0 || /^(docs|scripts|tests|instructions|verify|src|templates)/.test(token);
    }))).length;
  }
  ...
  function isTaskDecompositionCandidate(task) {
    ...
    var text = [
      ...
      task.work_steps || '',
      task.test_methods || '',
      task.review_personas || ''
    ].join(' ');
    var hasSpec = /(Spec|SPEC|仕様|設計|schema|brief|acceptance|criteria|要件|方針|調査|分析)/i.test(text);
    var hasImpl = /(Impl|IMPL|実装|backend|frontend|UI|コード|deploy|デプロイ|テスト|test|script|PWA|API)/i.test(text);
    var personaCount = countReviewPersonas(task);
    var fileCount = countChangedFiles(task);
    return (hasSpec && hasImpl) || personaCount >= 3 || fileCount >= 5 || /大規模|sub期待値[^0-9]{0,16}[1-9][0-9]/.test(text);
  }

  function chooseTaskDecompositionPattern(task) {
    var text = [
      task && task.goal || '',
      task && task.goal_steps || '',
      task && task.work_steps || ''
    ].join(' ');
    if (/architecture|アーキ|移行計画|互換性|旧コード|migration/i.test(text)) return 'architecture';
    if (/refactor|リファクタ|再構成|構造変更/i.test(text)) return 'refactor';
    if (/bug|バグ|不具合|再現|regression|回帰|fix|修正/i.test(text)) return 'bugfix';
    return 'feature';
  }
  ...
  function buildTaskDecompositionPayload(task) {
    ...
    var payload = [
      ...
      'po_inbox_id: ' + poInboxId,
      'created_at: ' + now,
      'po_intent: ' + sanitizeLine((task.t_id || '') + ' を親子タスクに分解'),
      ...
      '---',
      '',
      '# ' + (task.t_id || '') + ' task decomposition request',
      '',
      'PWA の「分解する」から生成。候補 pattern: ' + pattern,
      '',
      '## 実行候補',
      '',
      '`sh scripts/devs_task_decomposition_check.sh --task-id ' + (task.mission_id || '') + '`',
      '`sh scripts/devs_task_decomposition_apply.sh --task-id ' + (task.mission_id || '') + ' --pattern ' + pattern + ' --dry-run`',
      '',
      '## task 概要',
      '',
      'goal: ' + (task.goal || '(none)'),
  ...
  function openTaskDecompositionModal(task) {
    ...
    openPrefillModal('タスク分解指示を生成', built.payload, built.poInboxId + '.md');
    return true;
  }
  ...
  function renderProgressBoard() {
    ...
    sorted.forEach(function (t) {
      ...
      if (isTaskDecompositionCandidate(t)) {
        var actions = el('div', { class: 'assign-buttons task-decomposition-actions' });
        var decomposeButton = el('button', {
          class: 'ghost-btn',
          type: 'button',
          text: '分解する',
          'aria-label': (t.t_id || '') + ' を親子タスクに分解'
        });
        decomposeButton.addEventListener('click', function (event) {
